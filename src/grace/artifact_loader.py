...
def load_development_plan(path: str) -> DevelopmentPlan:
    ...
    for phase_el in root.findall("Phase"):
        ...
        for wave_el in phase_el.findall("Wave"):
            ...
            modules = _list(wave_el, "Modules/Module")
            allowed_write_scope = _list(wave_el, "AllowedWriteScope/File")
            frozen_scope = _list(wave_el, "FrozenScope/File")
            must_preserve = _list(wave_el, "MustPreserve/Item")
            verification = _list(wave_el, "Verification/Command")
